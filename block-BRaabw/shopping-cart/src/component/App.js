...
class App extends React.Component {
  ...
  handleSort = ({ target }) => {
    ...
    this.setState({ [name]: value, product: [] }, () => this.sortData());
  };
  ...
  getTotalPrice = (cart) => {
    ...
    return [
      SUBTOTAL.toFixed(2),
      installments,
      (SUBTOTAL / installments).toFixed(2),
    ];
  };
  ...
  sortData = () => {
    ...
    if (value === "H2L") {
      ...
      setTimeout(() => {
        this.setState({
          sorterdFilteredArray: filterProduct,
  ...
  render() {
    console.log("render");
    let displayProduct =
      this.state.sorterdFilteredArray.length === 0
        ? product
        : this.state.sorterdFilteredArray;
    return (
      <div className="flex">
        <Sidebar
          sizes={product}
        ...
        <main className="flex flex-col mt-14 w-2/3">
          <Head count={displayProduct.length} onChange={this.handleSort} />
...
const Sidebar = (props) => {
  let sizes = props.sizes
    .reduce((acc, cv) => acc.concat(cv.availableSizes), [])
    .filter((v, i, a) => a.indexOf(v) === i);
  let classes = "cursor-pointer bg-gray-200 p-4 m-3 pr-3 text-xs a";
  return (
    <div className="subpixel-antialiased text-normal p-14 h-screen w-80 ">
      <h3>Sizes:</h3>
        ...
        {sizes.map((size, i) => {
          ...
          return (
            <button
              ...
              className={activeClass + classes}
              style={{ heigth: "60px", width: "45px", borderRadius: "50%" }}
            >
              {size}
            </button>
          );
      ...
      <p className="mt-5 text-xs subpixel-antialiased">
        follow me on Github if you like this layout my username is rahulsharma522
      </p>
    </div>
  );
};
...
const Head = (props) => {
  return (
    <div className="flex justify-between subpixel-antialiased text-normal ml-2 px-2 ">
      <h4 className="pl-2">{props.count} Product(s) found</h4>
      <form>
        <label>Order By:</label>
        <select
          className="shadow leading-tight text-sm focus:none w-36 px-1 pr-0 py-2 mx-2 mr-1"
          name="select"
...
const ProductList = (props) => {
  return (
    <div className="w-full my-7 ml-2  flex flex-wrap items-baseline">
...
const ProductCard = ({
  ...
  return (
    <div className=" mb-10 mx-4 text-center w-52">
      {isFreeShipping && (
      ...
      <img
        ...
        src={`../static/products/${img}_1.jpg`}
      ...
      <div className="my-2">
        <h4
          className="text-sm font-normal  pt-4 px-4 mb-3"
          style={{
            paddingBottom: title.length > 4 && title.length < 24 ? "20px" : "",
          }}
        >
          {title}
        </h4>
      ...
      <div className="flex items-baseline ml-16">
        ...
        <p>
          <b className="text-3xl">{amount[0]}</b>
          {amount[1] ? "." + amount[1] : ".00"}
        </p>
      </div>
      <div className="flex ml-16 text-gray-500">
        ...
        <p className="font-bold">
          {installments > 0
            ? currencyFormat + "" + (price / installments).toFixed(2)
            : ""}
        </p>
      </div>
      <button
        ...
        className="bg-black hover:bg-yellow-400 text-white text-sm mt-3 font-normal subpixel-antialiased py-3 px-16 "
      >
        Add To Cart
      </button>
    </div>
    ...
    <div>
      {isCartOpen ? (
        <div className="flex fixed right-1 max-h-screen	">
          <div
            className=" text-white pt-3 pb-4 w-14 pl-5 h-12"
            style={{ backgroundColor: "rgb(27,26,32)" }}
          >
          ...
          >
            ...
            <div
              className="mt-10 overflow-y-scroll p-3 mx-3x"
              style={{ height: "63%", backgroundColor: "rgb(27,26,32)" }}
            >
              {cart.length === 0 ? (
            ...
            >
              <div className="flex justify-between items-center	mx-2 pt-2 text-right text-gray-700 px-5 my-1 ">
                ...
                <div className="mr-3">
                  <p className="font-normal text-2xl text-amber-500">
                    ${amount[0]}
                  </p>
                  <p className="text-gray-400">
                    OR UP TO {amount[1]} x $ {amount[2]}
                  </p>
                </div>
              </div>
              <div className="text-center mt-6 p-2">
                <button
                  onClick={() => alert(`SUBTOTAL = $${amount[0]}`)}
                  className="w-96 h-10 bg-black mb-5 text-white"
                >
                  CHECKOUT
                </button>
              </div>
        ...
        <div className="bg-black text-white absolute right-1 pr-8 pt-6 pb-6 pl-5">
          <button onClick={handleCartToggle}>
            ...
            <span className="absolute self-center top-14 left-14 bg-yellow-400 text-black rounded-full pl-2 pr-2">
              {" "}
              {cart.length}{" "}
            </span>
          </button>
    ...
    <>
      <div className="pl-52 pr-52 mt-12 flex">
        <div>
          ...
          <span className="absolute top-16 mt-5 ml-7 bg-yellow-400 text-black rounded-full pl-2 pr-2">
            {" "}
            {count}
          </span>
        </div>
...
const CartItemCard = (props) => {
  return (
    <div
      className="flex justify-between  border-t-2 border-black m-2 pt-2"
      style={{ backgroundColor: "rgb(27,26,32)" }}
    >
      <div className="flex">
        <img
          className="w-20 mr-3"
          src={`../static/products/${props.item.img}_2.jpg`}
          alt="photos"
        />
        <div className="mt-4">
      ...
      <div className="text-right mr-3">
        <button
          onClick={() => props.handleRemove(props.item.id, props.index)}
          className="font-bold not-italic text-lg"
        >
          X
        </button>
        <p className="my-1 mb-3 text-amber-400">${props.item.price}</p>
        <div>
          <span className="p-1 px-2 text-lg bg-black text-white">
            <button
              onClick={() => props.handleIncrement(props.item, props.index)}
            >
              +
            </button>
          </span>
          <span className="p-1 px-2 text-lg bg-black text-white">
            <button
              onClick={() => props.handleDecrement(props.item, props.index)}
